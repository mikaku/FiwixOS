...
build() {
	...
	case $# in
		7)
			;;
		*)
			echo "ERROR: wrong number of parameters ($#) in build($prg)."
			exit 1
			;;
	esac
	...
	if ! [ -f ${BUILDS}/${altprg}_${ver}_${arch}.ipk ] ; then
		...
		echo "File '${BUILDS}/${altprg}_${ver}_${arch}.ipk' already exists."
		return
	fi

	echo
	echo "$altprg-$ver completed!"
	echo
	sync ; sync
...
build libxml2 "" v2.9.14 i386 "" "Library providing XML and HTML support" "https//gitlab.gnome.org/GNOME/libxml2/-/wikis/home"
